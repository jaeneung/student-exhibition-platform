...
`lib/store.ts`는 프로젝트를 두 부분으로 나눠 저장합니다:

- **목록(metadata)**: 제목, 카테고리, 상태, 실행 링크 등 — 갤러리/`/manage`/`/my` 같은
  목록 화면이 실제로 쓰는 가벼운 필드만 담습니다.
- **콘텐츠(content)**: 업로드된 파일 내용(`uploadedFiles`/`uploadedHtml`, base64로 인코딩된
  이미지·문서 등)은 프로젝트별로 완전히 별도 저장 위치(Netlify Blobs의 별도
  키/로컬의 별도 파일)에 둡니다. 버전 기록의 각 버전도 마찬가지로 자기 콘텐츠를 따로
  저장합니다(`getProjectVersionContent`).

이렇게 나눈 이유는 실측 결과 때문입니다: 배포된 사이트에서 프로젝트 하나만 보여주는
상세 페이지가, 아무 데이터도 안 쓰는 페이지보다 갤러리 전체 목록과 거의 똑같은
1~1.2초를 더 썼습니다 — 렌더링 비용이 아니라 "목록 하나 읽으려고 모든 프로젝트의
업로드 파일까지 통째로 읽어오는" 구조 자체가 원인이라는 뜻입니다. 실제로 확인해 보니
프로젝트 23개 중 16개가 업로드 파일을 갖고 있었고, 그 base64 콘텐츠만 합쳐서 약 7-8MB —
JSON에 얹히면서 10MB 넘는 목록을 페이지 하나 볼 때마다 통째로 읽고 있었습니다. 목록
읽기(`getAllProjects`/`getPublicProjects`/`getProjectsByOwner`)는 이제 콘텐츠 없이 가벼운
버전만 반환하고, 실제 콘텐츠가 필요한 곳(편집 화면, `/files/**`·`/versions/**` 서빙
라우트)만 해당 프로젝트 하나의 콘텐츠를 그때 가서 따로 읽어옵니다.

이 구조로 바꾸기 전 이미 저장돼 있던 프로젝트들(콘텐츠가 목록 안에 그대로 들어있던
예전 형식)은 **처음 읽히는 순간 자동으로 마이그레이션**됩니다(`migrateInlineContentIfNeeded`)
— 콘텐츠를 먼저 별도 위치에 안전하게 써 넣은 뒤에만 목록에서 그 내용을 지우므로, 도중에
실패해도 원본 데이터가 사라지는 일은 없습니다. 배포 직후 여러 인스턴스가 동시에 이
마이그레이션을 시도할 수 있지만, 매번 같은 결과로 수렴하는 멱등적인 작업이라 안전합니다.

또한 `lib/store.ts`는 배포 환경에서 목록 읽기 결과를 15초간 메모리에 캐시합니다 — 같은
Netlify Function 인스턴스가 짧은 시간 안에 여러 요청을 처리할 때(방문자 여러 명이 비슷한
시각에 갤러리를 보는 경우 등) 반복되는 Blobs 조회를 없애 줍니다. 어떤 쓰기든 그 쓰기를
수행한 인스턴스의 캐시는 즉시 최신 상태로 갱신되므로, 그 인스턴스 자신은 자기 변경을
놓치지 않습니다 — 다른 동시 인스턴스(또는 캐시가 만료되기 전까지의 같은 인스턴스)만 최대
15초간 이전 상태를 볼 수 있습니다.
...
`/shorts`(`app/shorts/page.tsx`)는 "민평통 숏폼" 카테고리(`lib/types.ts`의
`SHORTS_CATEGORY`)만 모아 보여주는 독립된 페이지입니다. 헤더 메뉴에 별도 항목으로
있고, 메인 갤러리("/")에서는 이 카테고리를 아예 제외합니다 — 카테고리 필터 목록에도
나타나지 않고, 프로젝트 개수·태그/학년 필터에도 포함되지 않습니다
(`app/(gallery)/page.tsx`에서 `SHORTS_CATEGORY`를 명시적으로 걸러냅니다).

제출 과정 자체는 전혀 다르지 않습니다 — 학생은 평소처럼 `/submit`에서 로그인 후
제출하되, 카테고리에서 "민평통 숏폼"을 고르고 실행 링크 입력 모드로 유튜브 쇼츠·
인스타그램 릴스 등에 이미 올린 영상의 링크를 붙여넣습니다. 심사 대기 → 전시중 전환,
교사 검토, 학생 본인 수정/버전 기록까지 다른 프로젝트와 완전히 동일한 파이프라인을
씁니다 — 달라지는 것은 어디에 "보이는지"뿐입니다. 새 카테고리를 또 이런 식으로 분리하고
싶다면 `PROJECT_CATEGORIES`에 값을 추가하고, `SHORTS_CATEGORY`처럼 메인 갤러리에서
제외한 뒤 전용 페이지를 하나 더 만들면 됩니다.
...
`/manage`의 "🔗 링크 확인" 버튼(`components/CheckLinksButton.tsx`)을 누르면 그 시점에
등록된 모든 프로젝트의 실행 링크(`launchUrl`)에 실제로 요청을 보내서 응답하는지
확인합니다(`lib/linkCheck.ts`의 `isLinkReachable` — 6초 타임아웃, 2xx~3xx만 정상으로
간주). 응답하지 않는 프로젝트는 제목 옆에 ❗ 표시가 붙고, 마우스를 올리면 마지막으로
확인한 시각을 볼 수 있습니다.

**페이지를 열 때마다 자동으로 확인하지 않습니다** — 교사가 버튼을 눌렀을 때만 실행됩니다.
프로젝트가 많아지고 그중 일부가 외부 사이트라면, 매 방문마다 수십 개의 외부 요청을
보내는 것 자체가 오래 걸리는 작업이 되기 때문입니다(바로 위 "저장 구조와 응답 속도"에서
설명한, 페이지 로딩 속도를 지키기 위한 원칙과 같은 이유입니다). 확인 결과는
`Project.linkStatus`(`{checkedAt, ok}`)로 저장되고, 한 번 확인할 때 프로젝트 수만큼
`updateProject`를 각각 호출하는 대신 `updateLinkStatuses`로 **한 번에** 모아서
읽고-쓰기 때문에, 여러 프로젝트를 동시에 확인해도 서로의 결과를 덮어쓰지 않습니다.

업로드된 파일 기반 프로젝트(`/files/{id}/...`)도 예외 없이 같은 방식으로 확인합니다 —
제출 시점에 `launchUrl`이 이미 완전한 절대 주소로 저장되어 있어서, 외부 링크와 우리
사이트가 직접 서빙하는 콘텐츠를 구분할 필요 없이 똑같이 요청해 보면 됩니다.

정확도에 한계가 있다는 점은 알아두세요: 일부 사이트는 자동화된 요청을 차단하거나
(봇 차단, 리퍼러 검사 등) 일시적으로 느릴 수 있어, 실제로는 정상인데 "응답 없음"으로
표시될 수 있습니다. ❗ 표시는 "확실히 문제 있음"이 아니라 "확인해 볼 필요가 있음" 정도로
받아들이는 게 안전합니다.
