...
- **정식 데이터베이스 아님**: 로컬은 JSON 파일, 배포 환경은 Netlify Blobs를 쓰지만 둘 다
  ...
  트래픽이 커지면 실제 데이터베이스 도입이 필요합니다. 또한 프로젝트가 하나 늘 때마다
  업로드된 파일 내용(이미지/문서의 base64 등)까지 포함한 전체 목록을 통째로 읽고 쓰기
  때문에, 프로젝트 수와 업로드 용량이 커질수록 목록/상세 페이지 응답이 함께 느려집니다.
  `lib/store.ts`가 배포 환경에서 이 읽기 결과를 15초간 메모리에 캐시해(같은 Netlify
  Function 인스턴스가 재사용될 때 반복 조회 비용을 없애 줍니다) 당장의 체감 속도는
  개선했지만, 근본적인 해결은 업로드된 파일 내용을 프로젝트 목록과 분리된 저장 위치로
  빼내는 구조 변경입니다.
