...
**심사 대기중인 프로젝트도 교사는 미리 볼 수 있습니다.** `/manage`(목록)와
`/manage/[id]/edit`의 "👁️ 미리보기" 링크는 `project.launchUrl`을 새 탭으로 엽니다 —
학생이 업로드한 파일이 실제로 잘 열리는지 확인한 뒤 "전시중으로 전환" 버튼을 누를 수
있게 하기 위해서입니다. `/projects/[id]`(일반 방문자용 상세 페이지)는 위에서 설명한 대로
전시중 상태만 200을 반환하지만, 파일을 그대로 서빙하는 `/files/[id]/[[...path]]`는
상태와 무관하게 항상 동작합니다(id를 아는 것 자체를 비공개 외부 링크와 같은 신뢰 수준으로
취급하기 때문 — 자세한 내용은 그 라우트의 주석 참고) — 미리보기 기능은 이 기존 동작을
그대로 이용할 뿐, 별도의 접근 제어를 새로 추가하지 않습니다.
