...
- **📁 파일 업로드**: 완성된 프로젝트를 이 앱이 직접 호스팅합니다(최대 3MB).
  ...
  - 포스터나 카드뉴스처럼 결과물이 **이미지 한 장(.png/.jpg/.gif/.webp) 또는 PDF
    한 장**이라면 그 파일을 바로 올릴 수 있습니다. HTML/ZIP과 마찬가지로
    `/files/{프로젝트 id}`로 서빙되며, 이미지인 경우 대표 이미지를 비워두면 thum.io
    스크린샷 대신 업로드한 이미지 자체를 그대로 대표 이미지로 씁니다. 보안상
    `.svg`는 이 경로로는 받지 않습니다 — HTML처럼 `<script>`를 담을 수 있는 형식이라
...
방문자 갤러리에 노출되지 않는다는 점이 유일한 방어선입니다. 이미지(PNG/JPG/GIF/WebP)와
PDF는 `<script>`를 담을 수 없는 정적 형식이라 이 위험이 없지만, SVG는 예외라서
"이미지 업로드"로는 받지 않습니다(위 참고) — 그래도 모든 업로드는 동일하게 이 검토
절차를 거쳐야 전시됩니다.
...
- **콘텐츠 검수 없음**: 제출된 URL이나 이미지 URL의 안전성은 스킴(http/https)만
  ...
  PDF)도 마찬가지로 sandbox 없이 그대로 서빙되며, 안전성은 교사의 심사 절차에 의존합니다(위 "실행 링크"
