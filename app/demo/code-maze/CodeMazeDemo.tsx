...
import type { CodeMazeContent } from "./content";
...
export function CodeMazeDemo({ content }: { content: CodeMazeContent }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-lg flex-1 flex-col gap-6 px-4 py-10 sm:px-6">
      ...
      <div className="mx-auto grid w-40 grid-cols-3 gap-1">
        <div />
        <button type="button" onClick={() => tryMove(0, -1)} className="rounded-lg bg-zinc-900 py-2 text-white dark:bg-zinc-100 dark:text-zinc-900" aria-label={content.moveUpLabel}>↑</button>
        <div />
        <button type="button" onClick={() => tryMove(-1, 0)} className="rounded-lg bg-zinc-900 py-2 text-white dark:bg-zinc-100 dark:text-zinc-900" aria-label={content.moveLeftLabel}>←</button>
        <button type="button" onClick={() => tryMove(0, 1)} className="rounded-lg bg-zinc-900 py-2 text-white dark:bg-zinc-100 dark:text-zinc-900" aria-label={content.moveDownLabel}>↓</button>
        <button type="button" onClick={() => tryMove(1, 0)} className="rounded-lg bg-zinc-900 py-2 text-white dark:bg-zinc-100 dark:text-zinc-900" aria-label={content.moveRightLabel}>→</button>
      </div>

      <p className="text-center text-sm text-zinc-500">{format(content.moveCountLabel, { count: moves })}</p>

      {showQuiz && !quizDone && (
        <div role="dialog" aria-label={content.bonusQuizAriaLabel} className="rounded-2xl border border-amber-300 bg-amber-50 p-5 dark:border-amber-800 dark:bg-amber-950">
          <p className="mb-3 font-semibold text-amber-900 dark:text-amber-200">
            {content.bonusQuizPrefix}
            ...
            {content.quiz.options.map((opt, i) => (
              <button
                ...
                onClick={() => {
                  ...
                  setShowQuiz(false);
                  setQuizFeedback(
                    i === content.quiz.answerIndex
                      ? content.quizCorrect
                      : format(content.quizWrong, { answer: content.quiz.options[content.quiz.answerIndex] })
                  );
                }}
      ...
      {won && (
        <div role="status" className="rounded-2xl border border-emerald-300 bg-emerald-50 p-5 text-center dark:border-emerald-800 dark:bg-emerald-950">
          <p className="text-lg font-bold text-emerald-800 dark:text-emerald-200">
            {format(content.winMessage, { moves })}
          </p>
          <button
            type="button"
            onClick={reset}
