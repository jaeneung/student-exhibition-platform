...
const en: Dictionary = {
  ...
  form: {
    ...
    launchFileHint:
      "If your project is just one HTML file, upload that (.html). If it uses other files too, like images, zip up the whole folder and upload the .zip instead, or use '📂 Upload a Folder' below to upload it with no zipping at all — if a picture points to a path on your own computer, it won't show up in the gallery, so use one of these if your project has images. If your project is just a single image (.png, .jpg, .gif, .webp) or short video (.mp4, .webm, .mov), you can upload that file directly. Beyond that, most other file types — PDF, Word (.doc/.docx), Excel (.xls/.xlsx), PowerPoint (.ppt/.pptx), HWP (.hwp), CSV, TXT, and more — can also be uploaded as-is (except .svg). Up to 4MB — most videos are much bigger than that, so for a longer one, upload it to YouTube or similar and use that as your launch link instead.",
